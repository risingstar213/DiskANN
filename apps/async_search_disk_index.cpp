...
#define WARMUP false
#define COROUTINES_PER_THREAD 30
...
diskann::Task<void> async_single_search(
    std::shared_ptr<diskann::AsyncPQFlashIndex<T, LabelT>> index,
    const T *query, uint32_t recall_at, uint32_t L,
    uint64_t *result_ids, float *result_dists,
    uint32_t beamwidth, bool filtered_search = false, 
    LabelT filter_label = LabelT{}, bool use_reorder_data = false,
    diskann::QueryStats *stats = nullptr) {
    
    co_await index->async_cached_beam_search(
        query, recall_at, L, result_ids, result_dists,
...
{
    diskann::cout << "Async Search parameters: #threads: " << num_threads << ", ";
    if (beamwidth <= 0)
    ...
    if (WARMUP)
    {
        ...
        try {
            for (uint32_t i = 0; i < warmup_num; i++) {
                auto warmup_task = async_single_search<T, LabelT>(
            ...
            diskann::cout << "Warmup failed: " << e.what() << std::endl;
        }
    }
    ...
    for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++)
    {
        ...
        // TODO: 实现 beamwidth 优化（暂时使用固定值）
        optimized_beamwidth = beamwidth > 0 ? beamwidth : 4;
        ...
        try {
            // 使用协程逐个进行搜索（模拟 OpenMP 并行）
            std::vector<diskann::Task<void>> search_tasks;
            search_tasks.reserve(query_num);

            for (uint32_t i = 0; i < query_num; i++) {
                LabelT label_for_search = LabelT{};
                if (filtered_search) {
                ...
                search_tasks.push_back(async_single_search<T, LabelT>(
                    _pFlashIndex,
...
int main(int argc, char **argv)
{
    ...
    po::options_description desc{
        program_options_utils::make_program_description("async_search_disk_index", "Async searches on-disk DiskANN indexes using coroutines")};
    try
    {
    ...
    try
    {
        if (!query_filters.empty() && label_type == "ushort")
        {
            if (data_type == std::string("float"))
                return async_search_disk_index<float, uint16_t>(
                    metric, index_path_prefix, result_path_prefix, query_file, gt_file, num_threads, K, W,
                    num_nodes_to_cache, search_io_limit, Lvec, fail_if_recall_below, query_filters, use_reorder_data);
            else if (data_type == std::string("int8"))
                return async_search_disk_index<int8_t, uint16_t>(
                    metric, index_path_prefix, result_path_prefix, query_file, gt_file, num_threads, K, W,
                    num_nodes_to_cache, search_io_limit, Lvec, fail_if_recall_below, query_filters, use_reorder_data);
            else if (data_type == std::string("uint8"))
                return async_search_disk_index<uint8_t, uint16_t>(
                    metric, index_path_prefix, result_path_prefix, query_file, gt_file, num_threads, K, W,
                    num_nodes_to_cache, search_io_limit, Lvec, fail_if_recall_below, query_filters, use_reorder_data);
            else
            {
        ...
        {
            if (data_type == std::string("float"))
                return async_search_disk_index<float>(metric, index_path_prefix, result_path_prefix, query_file, gt_file,
                                                     num_threads, K, W, num_nodes_to_cache, search_io_limit, Lvec,
                                                     fail_if_recall_below, query_filters, use_reorder_data);
            else if (data_type == std::string("int8"))
                return async_search_disk_index<int8_t>(metric, index_path_prefix, result_path_prefix, query_file, gt_file,
                                                       num_threads, K, W, num_nodes_to_cache, search_io_limit, Lvec,
                                                       fail_if_recall_below, query_filters, use_reorder_data);
            else if (data_type == std::string("uint8"))
                return async_search_disk_index<uint8_t>(metric, index_path_prefix, result_path_prefix, query_file, gt_file,
                                                        num_threads, K, W, num_nodes_to_cache, search_io_limit, Lvec,
                                                        fail_if_recall_below, query_filters, use_reorder_data);
            else
            {
